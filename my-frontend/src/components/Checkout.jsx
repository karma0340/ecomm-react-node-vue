  ...
  useEffect(() => {
    const getClientSecret = async () => {
      if (form.paymentMethod === 'card' && cartItems.length > 0) {
        ...
        try {
          const res = await axios.post('http://localhost:3000/api/payment/create-payment-intent', { amount: amountInPaise });
      ...
      <Paper elevation={3} sx={{ p: 4 }}>
        ...
        <form onSubmit={handleOrder}>
          ...
          {form.paymentMethod === 'card' && (
            <Box mt={2} mb={2}>
              <Typography variant="subtitle2" mb={1}>Card Details</Typography>
...
function Checkout({ fetchUserAndCart }) {
  ...
  if (!cartItems.length) return (
    <Box mt={8} textAlign="center">
      ...
      <Button variant="contained" onClick={() => window.location = '/'}>Continue Shopping</Button>
    </Box>
  );
