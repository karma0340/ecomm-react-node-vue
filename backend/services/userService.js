...
class UserService {
  ...
  async getUserById(id) {
    try {
      console.log('getUserById called with id:', id); // Debug log
  ...
  async updateUser(id, updates) {
    try {
      ...
      // Check for username/email uniqueness if updating
  ...
  // For /me endpoint (get current user info)
  async getMe(id) {
    try {
      console.log('getMe called with id:', id); // Debug log

      const user = await User.findByPk(id, {
        attributes: ['id', 'username', 'email', 'name', 'role']
      });

      console.log('getMe result:', user); // Debug log
